...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Batch PNG/JPEG → JPG (doelgrootte in KB)")
        ...
        # Extra optie: ook JPEG/JPG meenemen
        self.allow_jpeg_input_var = tk.BooleanVar(value=False)
    ...
    def _build_ui(self):
        ...
        # Grid layout zodat knoppen (Start/Stop) altijd zichtbaar blijven, ook bij hoge DPI.
        self.rowconfigure(0, weight=1)
        self.columnconfigure(0, weight=1)
        root.grid(row=0, column=0, sticky="nsew")
        root.columnconfigure(0, weight=1)
        root.rowconfigure(0, weight=3)   # bovenste deel (lijst + knoppen)
        root.rowconfigure(3, weight=2)   # log
        ...
        ttk.Label(left, text="Geselecteerde bestanden (PNG/JPG/JPEG):").pack(anchor="w")
        self.listbox = tk.Listbox(left, selectmode=tk.EXTENDED, height=11)
        self.listbox.pack(fill="both", expand=True, pady=(6, 0))

        btns = ttk.Frame(top)
        btns.pack(side="right", fill="y", padx=(12, 0))

        ttk.Button(btns, text="Bestanden toevoegen…", command=self.add_files).pack(fill="x", pady=4)
        ...
        ttk.Checkbutton(input_row, text="Ook JPEG/JPG meenemen (naast PNG)", variable=self.allow_jpeg_input_var).pack(side="left")
        row(5, "Input:", input_row)
        ...
        row(6, "Resizing:", resize_row)

        # Run controls + progress
        run = ttk.Frame(root)
        run.grid(row=2, column=0, sticky="ew", pady=(4, 6))
    ...
    def add_files(self):
        allow_jpeg = bool(self.allow_jpeg_input_var.get())
        if allow_jpeg:
            ftypes = [
        ...
        for p in paths:
            ...
            if ext == ".png" or (allow_jpeg and ext in (".jpg", ".jpeg")):
    ...
    def start(self):
        ...
        if not self.files:
            messagebox.showwarning("Geen bestanden", "Selecteer eerst één of meer bestanden (PNG, JPG, JPEG).")
            return
        if not self.out_dir:
        ...
        self._log("Start… (PNG → JPG, optioneel ook JPG/JPEG → JPG)")
